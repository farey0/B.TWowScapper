...
function g_initHeader(B) {
  ...
  for (var G = 0, J = mn_path.length; G < J; ++G) {
    ...
    F.appendChild(document.createTextNode(mn_path[G][1].charAt(0)));
    I.appendChild(F);
    I.appendChild(document.createTextNode(mn_path[G][1].substr(1)));
    L.appendChild(I);
    L.appendChild(D);
    E.appendChild(L);
    H.appendChild(E);
  }
  document.getElementById("toptabs-right-generic").appendChild(H);
  var A = document.getElementById("menu-buttons-generic");
  ...
  var M = document.getElementById("live-search-generic");
...
function g_submit_live_search() {
  document.getElementById("live-search-generic").value = document.getElementById("live-search-generic").value.trim();
  document.getElementById("header-search-form").submit();
}
function g_initLanguageChanger() {
  var A = document.getElementById("language-changer");
  if (!A) {
    return;
  }
  A.menu = [[0, "English", g_locale.id != 0 ? "?locale=0" : null]];
  A.menu.rightAligned = 1;
  if (g_locale.id != 25) {
    A.menu[{0: 0, 8: 1}[g_locale.id]].checked = 1;
  }
  A.onmouseover = Menu.show;
  A.onmouseout = Menu.hide;
}
function g_initPath(K, D) {
  var G = mn_path, H = null, C = null, L = 0, I = document.getElementById("main-precontents"), A = ce("div");
  A.className = "path";
  if (D != null) {
    var J = ce("div");
    ...
    if (D) {
      ...
      M.appendChild(document.createTextNode(LANG.fihide));
    } else {
      M.className = "disclosure-off";
      M.appendChild(document.createTextNode(LANG.fishow));
    }
    J.appendChild(M);
    I.appendChild(J);
  }
  for (var F = 0; F < K.length; ++F) {
    ...
    M.appendChild(document.createTextNode(G[4] == null ? G[1] : G[4]));
    if (F == 0) {
      M.menu = mn_path;
    } else {
      M.menu = H[3];
...
function g_appendReputation(G, F, D, E) {
  ...
  var A = ce("del");
  A.appendChild(document.createTextNode(g_reputation_standings[F]));
  H.appendChild(A);
  var C = ce("ins");
...
function g_convertRatingToPercent(E, A, D) {
  var C = {12: 1.5, 13: 12, 14: 15, 15: 5, 16: 10, 17: 10, 18: 8, 19: 14, 20: 14, 21: 14, 22: 0, 23: 0, 24: 0, 25: 0, 26: 0, 27: 0, 28: 10, 29: 10, 30: 10, 31: 10, 32: 14, 33: 0, 34: 0, 35: 25, 36: 10, 37: 2.5};
  if (E < 10) {
    E = 10;
  } else {
    if (E > 70) {
  ...
  if (C[A] == null) {
    ...
    f = C[A];
    if (E >= 1 && E <= 59) {
      B = D / f / (0.019230769230769232 * E - 0.15384615384615385);
    } else {
      if (E >= 60 && E <= 70) {
        B = D / f * (-0.036585365853658534 * E + 3.1951219512195124);
      } else {
        B = 0;
      }
    }
...
function g_getMoneyHtml(C) {
  ...
  if (C >= 1e4) {
    B = 1;
    A += '<span class="moneygold">' + Math.floor(C / 1e4) + "</span>";
    C %= 1e4;
  }
  if (C >= 100) {
    if (B) {
...
g_getPatchVersion.T = [11535408e5, 11592432e5, 1160712e6];
function g_expandSite() {
  de(document.getElementById("sidebar"));
  de(document.getElementById("header-ad"));
  document.getElementById("wrapper").className = "noads";
  var A = document.getElementById("contribute-ad");
  if (A) {
    de(A);
  }
}
...
g_getLocaleFromDomain.L = {fr: 2, de: 3, es: 6, wotlk: 25};
function g_getIdFromTypeName(A) {
  var B = g_getIdFromTypeName.L;
  return B[A] ? B[A] : -1;
}
g_getIdFromTypeName.L = {npc: 1, object: 2, item: 3, itemset: 4, quest: 5, spell: 6, zone: 7, faction: 8};
function g_getIngameLink(color, entry, name) {
  prompt("Copy/paste the following to your in-game chat window:", '/script DEFAULT_CHAT_FRAME:AddMessage("\\124c' + color + "\\124Hitem:" + entry + ":0:0:0:0:0:0:0:0\\124h[" + name.replace(/"/g, '\\"') + ']\\124h\\124r");');
}
function g_userDescription() {
  ...
  if (B) {
    if (D) {
      C.appendChild(document.createTextNode(LANG.user_nodescription2));
    } else {
      C.appendChild(document.createTextNode(LANG.user_nodescription));
    }
  }
  if (D) {
    ...
    if (B) {
      A.appendChild(document.createTextNode(LANG.user_composeone));
    } else {
      A.appendChild(document.createTextNode(LANG.user_editdescription));
    }
    C.appendChild(E);
    C.appendChild(A);
  }
...
function co_addYourComment() {
  ...
  var A = document.forms.addcomment.getElementsByTagName("textarea")[0];
  A.focus();
}
function co_cancelReply() {
  document.getElementById("replybox-generic").style.display = "none";
...
function ss_appendSticky() {
  var J = document.getElementById("infobox-sticky");
...
function g_ajaxIshRequest(A) {
  ...
  B.appendChild(ce("script", {type: "text/javascript", src: A}));
}
var Menu = {iframes: [], divs: [], selection: [], show: function () {
  ...
  var l = ce("table"), A = ce("tbody"), E = ce("tr"), I = ce("td"), Q = ce("div"), G = ce("div");
  var S = 999;
  var P = g_getWindowSize(), C = g_getScroll(), J = P.w, T = P.h, B = C.x, o = C.y;
  if (g > 0) {
    if (26 * V.length > T - 25 - D) {
      for (var X = 2; X < 4; ++X) {
  ...
  for (var X = 0, e = V.length; X < e; ++X) {
    if (V[X][0] == null) {
      ...
      ns(Y);
      Y.appendChild(document.createTextNode(V[X][1]));
      Y.d = n;
      ...
      if (V[X][5] != null) {
        ...
        if (V[X].checked) {
          ...
          K.appendChild(document.createTextNode(V[X][1]));
          F.appendChild(K);
        } else {
          F.appendChild(document.createTextNode(V[X][1]));
        }
      } else {
        if (V[X].checked) {
    ...
    if (H++ == S) {
      ...
      if (!Browser.ie6) {
        var R = ce("p");
        R.appendChild(ce("em"));
        R.appendChild(ce("var"));
        R.appendChild(ce("strong"));
        R.appendChild(Q);
        I.appendChild(R);
      } else {
        I.appendChild(Q);
  ...
  if (!Browser.ie6) {
    if (S != 999) {
      ...
      n.appendChild(ce("em"));
      n.appendChild(ce("var"));
      n.appendChild(ce("strong"));
      I.appendChild(Q);
    }
  } else {
    I.appendChild(Q);
  ...
  n.style.display = "";
  var K = l.offsetWidth, W = l.offsetHeight;
  if (V.rightAligned) {
    M = M + N - K;
  } else {
    if (M + N + K + 15 > J) {
  ...
  for (var B = A; B < Menu.selection.length; ++B) {
    if (Menu.selection[B] != -1) {
      d = Menu.divs[B].getElementsByTagName("a")[Menu.selection[B]];
  ...
  for (var C = 0, A = E.length; C < A; ++C) {
    ...
    D.appendChild(document.createTextNode(E[C][1]));
    B.appendChild(D);
    F.appendChild(B);
  }
}, explode: function (F) {
...
function Tabs(A) {
  ...
  this.oldMode = Browser.geckoVersion > 2e7 && Browser.geckoVersion <= 20060414;
  this.selectedTab = -1;
  this.uls = [];
  this.tabs = [];
  this.nShows = 0;
...
Tabs.prototype = {add: function (A, D) {
  ...
  this.forceScroll = 1;
  this.uls[this.oldMode ? 0 : 2].getElementsByTagName("a")[A].onclick({}, true);
  ...
  for (var D = 0; D <= A; ++D) {
    B = this.uls[D].getElementsByTagName("a");
    if (this.selectedTab != -1) {
  ...
  if (this.oldMode) {
    ...
    for (var G = 0; G < this.tabs.length; ++G) {
      ...
      K.appendChild(document.createTextNode(D.caption));
      M.appendChild(C);
      M.appendChild(K);
      H.appendChild(M);
    }
    ...
    for (var G = 0; G < this.tabs.length; ++G) {
      ...
      for (var F = 0; F <= 3; ++F) {
        ...
        if (!Browser.ie6) {
          I = ce("div");
          I.appendChild(document.createTextNode(D.caption));
          K.appendChild(I);
        }
        b.appendChild(document.createTextNode(D.caption));
        K.appendChild(b);
        E.appendChild(K);
        this.uls[F].appendChild(E);
      }
    }
  ...
  var A = this.oldMode ? 0 : 3;
  for (var D = 0; D <= A; ++D) {
    _ = this.uls[D].getElementsByTagName("a");
...
Tabs.onShow = function (E, D) {
  var B;
  if (D) {
    document.getElementById("tab-" + D.id).style.display = "none";
  }
  B = document.getElementById("tab-" + E.id);
  B.style.display = "";
...
var Icon = {sizes: ["small", "medium", "large"], create: function (A, J, I, D, B, H) {
  var G;
...
var Tooltip = {clip: "main-contents", create: function (G) {
  ...
  Tooltip.icon.appendChild(ce("div"));
  E.appendChild(Tooltip.icon);
  E.appendChild(J);
  return E;
}, fix: function (D, B, E) {
  var C = D.getElementsByTagName("table")[0], G = C.getElementsByTagName("td")[0], F = G.childNodes;
  ...
  if (!Tooltip.tooltip) {
    ...
    A.appendChild(B);
    Tooltip.tooltip = B;
    Tooltip.tooltipTable = B.getElementsByTagName("table")[0];
    Tooltip.tooltipTd = B.getElementsByTagName("td")[0];
    if (Browser.ie6) {
      B = ce("iframe");
      B.src = "javascript:1;";
      B.frameBorder = 0;
...
function Listview(A) {
  ...
  if (this.id) {
    ...
    if (this.parent) {
      ...
      this.container = document.getElementById(K);
    }
  } else {
    return;
  }
...
Listview.prototype = {initialize: function () {
  ...
  for (var E = 0, G = this.visibility.length; E < G; ++E) {
    ...
    F.appendChild(document.createTextNode(D.name));
    A.appendChild(F);
    J.appendChild(A);
    B.appendChild(J);
    C.appendChild(B);
  ...
  C.appendChild(this.navTop);
  D.appendChild(this.navBot);
  C.appendChild(A);
  D.appendChild(B);
  if (this.mode == 1) {
    C.appendChild(this.createCbBar());
    D.appendChild(this.createCbBar());
  }
  this.bandTop = C;
  this.bandBot = D;
  this.updateNav();
  ...
  this.container.appendChild(this.bandTop);
  this.container.appendChild(E);
  this.container.appendChild(this.bandBot);
}, createNav: function () {
  var B = ce("div");
  B.className = "listview-nav";
  var D = ce("a"), C = ce("a"), A = ce("a"), I = ce("a");
  D.href = C.href = A.href = I.href = "javascript:;";
  D.appendChild(document.createTextNode("«" + LANG.lvpage_first));
  C.appendChild(document.createTextNode("‹" + LANG.lvpage_previous));
  A.appendChild(document.createTextNode(LANG.lvpage_next + "›"));
  I.appendChild(document.createTextNode(LANG.lvpage_last + "»"));
  ns(D);
  ns(C);
  ns(A);
  ns(I);
  ...
  E.appendChild(document.createTextNode("a"));
  H.appendChild(G);
  H.appendChild(document.createTextNode(LANG.hyphen));
  H.appendChild(F);
  H.appendChild(document.createTextNode(LANG.lvpage_of));
  H.appendChild(E);
  B.appendChild(D);
  B.appendChild(C);
  B.appendChild(H);
  ...
  var F = ce("div");
  var C = ce("div");
  var B = ce("a"), A = ce("a"), E = ce("a");
  B.href = A.href = E.href = "javascript:;";
  B.appendChild(document.createTextNode("All"));
  A.appendChild(document.createTextNode("None"));
  E.appendChild(document.createTextNode("Inverse"));
  ...
  ns(A);
  ns(E);
  C.appendChild(document.createTextNode("Select: "));
  C.appendChild(B);
  C.appendChild(document.createTextNode(LANG.comma));
  C.appendChild(A);
  C.appendChild(document.createTextNode(LANG.comma));
  C.appendChild(E);
  ...
  if (B > 0) {
    ...
    if (this.mode == 2) {
      for (var E = G; E < A; ++E) {
        this.mainDiv.appendChild(this.getDiv(E));
      }
    } else {
      if (this.mode == 3) {
        var C = 0, H = ce("tr");
        for (var E = G; E < A; ++E) {
          H.appendChild(this.getCell(E));
          if (++C == this.nItemsPerRow) {
            this.tbody.appendChild(H);
            if (E + 1 < A) {
              H = ce("tr");
            }
            C = 0;
        ...
        for (var E = G; E < A; ++E) {
          this.tbody.appendChild(this.getRow(E));
        }
      }
    }
  } else {
  ...
  if (A == -1) {
    B.appendChild(document.createTextNode(LANG.lvnodata));
  }
  ...
  for (var D = 0, E = this.visibility.length; D < E; ++D) {
    ...
    if (A.compute) {
      ...
      if (I[A.value] != null) {
        B.appendChild(document.createTextNode(I[A.value]));
      } else {
        H = -1;
      }
    }
    if (H != -1 && H != null) {
      B.insertBefore(document.createTextNode(H), B.firstChild);
    }
    F.appendChild(B);
  }
  ...
  var J = this.nItemsPerPage, H = this.rowOffset, D = this.data.length;
  var B = 0, F = 0, C = 0, I = 0;
  if (J) {
    if (H > 0) {
      F = 1;
...
Listview.cbIeFix = function () {
  var D = this.tbody.getElementsByTagName("tr");
...
Listview.extraCols = {cost: {id: "cost", name: LANG.cost, compute: function (A, B) {
  ...
  if (B.outof) {
    ...
    A.appendChild(document.createTextNode(sprintf(LANG.lvdrop_outof, B.outof)));
    C.appendChild(A);
  }
  return B.count;
...
Listview.funcBox = {createSimpleCol: function (C, D, A, B) {
  ...
  for (var D = 0, B = E.location.length; D < B; ++D) {
    if (D > 0) {
      F.appendChild(document.createTextNode(LANG.comma));
    }
    var A = E.location[D];
    if (A == -1) {
      F.appendChild(document.createTextNode(LANG.ellipsis));
    } else {
      var C = ce("a");
      C.className = "q1";
      C.href = "?zone=" + A;
      C.appendChild(document.createTextNode(g_zones[A]));
      F.appendChild(C);
    }
  }
}, createCenteredIcons: function (I, C, L) {
  if (I != null) {
    var H = ce("div"), A = ce("div");
    if (L) {
      var G = ce("div");
  ...
  if (K == -1 || K == 0) {
    ...
    J.appendChild(document.createTextNode(LANG.compose_mode));
    var P = ce("a");
    P.className = "selected";
    P.onclick = Listview.funcBox.coModeLink.bind(P, 1, K);
    P.href = "javascript:;";
    P.appendChild(document.createTextNode(LANG.compose_edit));
    J.appendChild(P);
    J.appendChild(document.createTextNode("|"));
    var N = ce("a");
    N.onclick = Listview.funcBox.coModeLink.bind(N, 2, K);
    N.href = "javascript:;";
    N.appendChild(document.createTextNode(LANG.compose_preview));
    J.appendChild(N);
    L.appendChild(J);
  }
  var A = ce("div");
  ...
  if (K != -1 && K != 0) {
    var E = ce("h3"), W = ce("a"), O = ce("div"), V = ce("div");
    ...
    W.appendChild(document.createTextNode(LANG.compose_livepreview));
    E.appendChild(W);
    W.href = "javascript:;";
    W.onclick = function () {
      D(1);
  ...
  var S = [{id: "b", title: LANG.markup_b, pre: "[b]", post: "[/b]"}, {id: "i", title: LANG.markup_i, pre: "[i]", post: "[/i]"}, {id: "u", title: LANG.markup_u, pre: "[u]", post: "[/u]"}, {id: "s", title: LANG.markup_s, pre: "[s]", post: "[/s]"}, {id: "small", title: LANG.markup_small, pre: "[small]", post: "[/small]"}, {id: "url", title: LANG.markup_url, onclick: function () {
  ...
  H.appendChild(ce("br"));
  if (K == 4) {
    H.appendChild(document.createTextNode(sprintf(LANG.compose_limit2, F)));
  } else {
    H.appendChild(document.createTextNode(sprintf(LANG.compose_limit, F)));
  }
  L.appendChild(H);
  return I;
  ...
  var C = this, I = Listview.funcBox.coGetCharLimit(F), G = C.value.length > I ? C.value.substring(0, I) : C.value;
  if (F == 4) {
    var H;
    if ((H = G.indexOf("\n")) != -1 && (H = G.indexOf("\n", H + 1)) != -1 && (H = G.indexOf("\n", H + 1)) != -1) {
      G = G.substring(0, H);
  ...
  var D = Markup.toHtml(G, {mode: Markup.MODE_COMMENT});
  if (D) {
    A.innerHTML = D;
  } else {
  ...
  if (D) {
    var A = E.divEdit.getElementsByTagName("textarea")[0];
    ...
    if (A.value != E.body) {
      ...
      E.divBody.innerHTML = Markup.toHtml(A.value.length > 7500 ? A.value.substring(0, 7500) : A.value, {mode: Markup.MODE_COMMENT});
      E.body = A.value;
      if (B == -1) {
        new Ajax("?forums=editpost&id=" + E.id, {method: "POST", params: "body=" + urlencode(E.body)});
      } else {
        new Ajax("?comment=edit&id=" + E.id, {method: "POST", params: "body=" + urlencode(E.body)});
      }
    }
  }
  ...
  array_walk(this.parentNode.getElementsByTagName("a"), function (J) {
    J.className = "";
  });
  this.className = "selected";
  var C = this.parentNode.parentNode.getElementsByTagName("textarea")[0], H = C.parentNode, E = H.previousSibling;
  if (A == 4) {
  ...
  switch (D) {
    ...
    case 2:
      ...
      E.innerHTML = Markup.toHtml(F, {mode: B});
      E.style.display = "";
      break;
  }
}, coReply: function (B) {
  document.forms.addcomment.elements.replyto.value = B.replyTo;
  var A = document.getElementById("replybox-generic");
  A.getElementsByTagName("span")[0].innerHTML = B.user;
  A.style.display = "";
  co_addYourComment();
}, coValidate: function (A, B) {
  B |= 0;
  if (B == 1 || B == -1) {
    if (A.value.replace(/(^\s*|\s*$)/g, "").length < 1) {
      alert(LANG.message_forumposttooshort);
      return false;
    }
  } else {
    if (A.value.replace(/(^\s*|\s*$)/g, "").length < 10) {
  ...
  if (A >= 1e4) {
    ...
    I.appendChild(document.createTextNode(Math.floor(A / 1e4)));
    E.appendChild(I);
    A %= 1e4;
  }
  if (A >= 100) {
    ...
    I.appendChild(document.createTextNode(Math.floor(A / 100)));
    E.appendChild(I);
    A %= 100;
  }
  if (A >= 1 || F != null) {
  ...
  if (J != null && J != 0) {
    ...
    I.appendChild(document.createTextNode(number_format(Math.abs(J))));
    E.appendChild(I);
  }
  if (K >= 1) {
    if (H) {
    ...
    I.onmousemove = Tooltip.cursorUpdate;
    I.onmouseout = Tooltip.hide;
    I.appendChild(document.createTextNode(number_format(K)));
    E.appendChild(I);
  }
  if (B != null) {
...
Listview.templates = {faction: {sort: [1], nItemsPerPage: -1, columns: [{id: "name", name: LANG.name, align: "left", value: "name", compute: function (C, D) {
  var A = ce("a");
...
}}, {id: "group", name: LANG.group, value: "group", compute: function (A, B) {
  if (A.group != null) {
    return A.group;
  }
}}, {id: "side", name: LANG.side, width: "10%", compute: function (B, C) {
  if (B.side) {
    var A = ce("span");
    A.className = B.side == 1 ? "alliance-icon" : "horde-icon";
    A.appendChild(document.createTextNode(g_sides[B.side]));
    C.appendChild(A);
  }
}, sortFunc: function (B, A, C) {
  ...
  return "?faction=" + A.id;
}}, item: {sort: [1], columns: [{id: "name", name: LANG.name, align: "left", span: 2, value: "name", compute: function (D, G, E) {
  var C = ce("td");
  C.style.width = "1px";
  C.style.padding = "0";
  ...
  C.appendChild(g_items.createIcon(D.id, 1, B, F));
  E.appendChild(C);
  ...
  A.appendChild(document.createTextNode(D.name.substring(1)));
  G.appendChild(A);
}}, {id: "level", name: LANG.level, width: "7%", value: "level", sortFunc: function (B, A, C) {
  return strcmp(B.level, A.level) || strcmp(B.reqlevel, A.reqlevel);
}}, {id: "reqlevel", name: LANG.req, tooltip: LANG.tooltip_reqlevel, width: "7%", value: "reqlevel", compute: function (A, B) {
  if (A.reqlevel > 1) {
    return A.reqlevel;
  }
}, sortFunc: function (B, A, C) {
  return strcmp(B.reqlevel, A.reqlevel) || strcmp(B.level, A.level);
}}, {id: "dps", name: LANG.dps, width: "10%", value: "dps", compute: function (A, B) {
  return (A.dps || 0).toFixed(1);
}, hidden: true}, {id: "speed", name: LANG.speed, width: "10%", value: "speed", compute: function (A, B) {
  return (A.speed || 0).toFixed(2);
}, hidden: true}, {id: "armor", name: LANG.armor, width: "10%", value: "armor", compute: function (A, B) {
  if (A.armor > 0) {
    return A.armor;
  }
}, hidden: true}, {id: "slot", name: LANG.slot, width: "10%", compute: function (A, B) {
  return g_item_slots[A.slot];
}, sortFunc: function (B, A, C) {
  return strcmp(g_item_slots[B.slot], g_item_slots[A.slot]);
}, hidden: true}, {id: "slots", name: LANG.slots, width: "10%", value: "nslots", hidden: true}, {id: "skill", name: LANG.skill, width: "10%", value: "skill", hidden: true}, {id: "source", name: LANG.source, width: "12%", compute: function (C, D) {
  if (C.source != null) {
    for (var B = 0, A = C.source.length; B < A; ++B) {
      if (B > 0) {
        D.appendChild(document.createTextNode(LANG.comma));
      }
      D.appendChild(document.createTextNode(g_sources[C.source[B]]));
    }
  }
}, sortFunc: function (B, A, C) {
  return Listview.funcBox.assocArrCmp(B.source, A.source, g_sources);
}}, {id: "type", name: LANG.type, width: "14%", compute: function (C, D) {
  ...
  A.href = B;
  A.appendChild(document.createTextNode(Listview.funcBox.getItemType(C.classs, C.subclass, C.subsubclass)));
  D.appendChild(A);
  ...
  return "?item=" + A.id;
}}, itemset: {sort: [1], nItemsPerPage: 75, columns: [{id: "name", name: LANG.name, align: "left", value: "name", compute: function (B, D) {
  var A = ce("a");
  A.className = "q" + (6 - parseInt(B.name.charAt(0)));
  A.style.fontFamily = "Verdana, sans-serif";
  A.href = this.template.getItemLink(B);
  A.appendChild(document.createTextNode(B.name.substring(1)));
  D.appendChild(A);
  if (B.note) {
    var C = ce("div");
    C.className = "small";
    C.appendChild(document.createTextNode(g_itemset_notes[B.note]));
    D.appendChild(C);
  }
}}, {id: "level", name: LANG.level, compute: function (A, B) {
...
}}, {id: "pieces", name: LANG.pieces, compute: function (A, B) {
  B.style.padding = "0";
  ...
  return strcmp(D, C);
}}, {id: "type", name: LANG.type, compute: function (A, B) {
  return g_itemset_types[A.type];
}, sortFunc: function (B, A, C) {
  return strcmp(g_itemset_types[B.type], g_itemset_types[A.type]);
}}, {id: "classes", name: LANG.classes, compute: function (C, D) {
  if (C.classes != null) {
    for (var B = 0, A = C.classes.length; B < A; ++B) {
      if (B > 0) {
        D.appendChild(document.createTextNode(LANG.comma));
      }
      D.appendChild(document.createTextNode(g_chr_classes[C.classes[B]]));
    }
  }
}, sortFunc: function (B, A, C) {
  ...
  return "?itemset=" + A.id;
}}, npc: {sort: [1], nItemsPerPage: 100, columns: [{id: "name", name: LANG.name, align: "left", value: "name", compute: function (B, D) {
  var A = ce("a");
  A.style.fontFamily = "Verdana, sans-serif";
  A.href = this.template.getItemLink(B);
  ...
  if (B.tag != null) {
    var C = ce("div");
    C.className = "small";
    C.appendChild(document.createTextNode("<" + B.tag + ">"));
    D.appendChild(C);
  }
}}, {id: "level", name: LANG.level, width: "10%", compute: function (A, C) {
  if (A.classification) {
    var B = ce("div");
    B.className = "small";
    B.appendChild(document.createTextNode(g_npc_classifications[A.classification]));
    C.appendChild(B);
  }
...
}}, {id: "location", name: LANG.location, compute: function (A, B) {
  return Listview.funcBox.location(A, B);
}, sortFunc: function (B, A, C) {
  return Listview.funcBox.assocArrCmp(B.location, A.location, g_zones);
}}, {id: "react", name: LANG.react, width: "10%", value: "react", compute: function (B, G) {
  if (B.react == null) {
    return -1;
  }
  ...
  for (var A = 0; A < 2; ++A) {
    if (B.react[A] != null) {
      ...
      E.appendChild(document.createTextNode(D[A]));
      G.appendChild(E);
    }
  }
}}, {id: "petfamily", name: LANG.petfamily, width: "12%", compute: function (B, C) {
  C.className = "q1";
  var A = ce("a");
  A.href = "?spells=-3." + B.family;
  A.appendChild(document.createTextNode(g_spell_skills[B.family]));
  C.appendChild(A);
}, sortFunc: function (B, A, C) {
  return strcmp(g_spell_skills[B.family], g_spell_skills[A.family]);
}, hidden: 1}, {id: "type", name: LANG.type, width: "12%", compute: function (B, C) {
  C.className = "small q1";
  var A = ce("a");
  A.href = "?npcs=" + B.type;
  A.appendChild(document.createTextNode(g_npc_types[B.type]));
  C.appendChild(A);
}, sortFunc: function (B, A, C) {
  return strcmp(g_npc_types[B.type], g_npc_types[A.type]);
}}], getItemLink: function (A) {
  return "?npc=" + A.id;
}}, object: {sort: [1], nItemsPerPage: 100, columns: [{id: "name", name: LANG.name, align: "left", value: "name", compute: function (B, C) {
  var A = ce("a");
  A.style.fontFamily = "Verdana, sans-serif";
  A.href = this.template.getItemLink(B);
  A.appendChild(document.createTextNode(B.name));
  ...
  return Listview.funcBox.assocArrCmp(B.location, A.location, g_zones);
}}, {id: "skill", name: LANG.skill, width: "10%", value: "skill", hidden: true}, {id: "type", name: LANG.type, width: "12%", compute: function (B, C) {
  C.className = "small q1";
  var A = ce("a");
  A.href = "?objects=" + B.type;
  A.appendChild(document.createTextNode(g_object_types[B.type]));
  C.appendChild(A);
}, sortFunc: function (B, A, C) {
  return strcmp(g_object_types[B.type], g_object_types[A.type]);
}}], getItemLink: function (A) {
  return "?object=" + A.id;
}}, quest: {sort: [1, 2], nItemsPerPage: 100, columns: [{id: "name", name: LANG.name, align: "left", value: "name", compute: function (B, C) {
  var A = ce("a");
  A.style.fontFamily = "Verdana, sans-serif";
  A.href = this.template.getItemLink(B);
  A.appendChild(document.createTextNode(B.name));
  C.appendChild(A);
}}, {id: "level", name: LANG.level, width: "7%", compute: function (A, C) {
  if (A.type || A.daily) {
    var B = ce("div");
    B.className = "small";
    B.style.whiteSpace = "nowrap";
    if (A.type && A.daily) {
      B.appendChild(document.createTextNode(sprintf(LANG.lvquest_daily, g_quest_types[A.type])));
    } else {
      if (A.daily) {
        B.appendChild(document.createTextNode(LANG.daily));
      } else {
        if (A.type) {
          B.appendChild(document.createTextNode(g_quest_types[A.type]));
        }
      }
    }
    C.appendChild(B);
...
}}, {id: "reqlevel", name: LANG.req, tooltip: LANG.tooltip_reqlevel, width: "7%", value: "reqlevel", sortFunc: function (B, A, C) {
  return strcmp(B.reqlevel, A.reqlevel) || strcmp(B.level, A.level);
}}, {id: "side", name: LANG.side, width: "10%", compute: function (A, C) {
  if (A.side) {
    var B = ce("span");
    if (A.side == 1) {
    ...
    B.appendChild(document.createTextNode(g_sides[A.side]));
    C.appendChild(B);
  } else {
    return -1;
  }
  ...
  return strcmp(g_sides[B.side], g_sides[A.side]);
}}, {id: "rewards", name: LANG.rewards, width: "25%", compute: function (B, F) {
  ...
  if (B.xp > 0 || B.money > 0) {
    ...
    if (B.xp > 0) {
      C.appendChild(document.createTextNode(sprintf(LANG.lvquest_xp, B.xp) + (B.money > 0 ? " + " : "")));
    }
    if (B.money > 0) {
...
}}, {id: "reputation", name: LANG.reputation, width: "14%", value: "id", hidden: true}, {id: "category", name: LANG.category, width: "16%", compute: function (B, C) {
  if (B.category != 0) {
    C.className = "small q1";
    var A = ce("a");
    A.href = "?quests=" + B.category2 + "." + B.category;
    A.appendChild(document.createTextNode(Listview.funcBox.getQuestCategory(B.category)));
    C.appendChild(A);
  }
}, sortFunc: function (B, A, D) {
  ...
  return "?quest=" + A.id;
}}, spell: {sort: [1, 2], columns: [{id: "name", name: LANG.name, align: "left", span: 2, value: "name", compute: function (F, C, J) {
  var E = ce("td"), M;
  E.style.width = "44px";
  E.style.padding = "0";
  ...
  L.appendChild(document.createTextNode(F.name.substring(1)));
  A.appendChild(L);
  ...
  if (F.races != null) {
    ...
    if (K == "1,3,4,7,11") {
      H.appendChild(document.createTextNode(g_sides[1]));
    } else {
      if (K == "2,5,6,8,10") {
        H.appendChild(document.createTextNode(g_sides[2]));
      } else {
        for (var E = 0, G = F.races.length; E < G; ++E) {
          if (E > 0) {
            H.appendChild(document.createTextNode(LANG.comma));
          }
          H.appendChild(document.createTextNode(g_chr_races[F.races[E]]));
        }
      }
    }
    A.appendChild(H);
...
}, hidden: true}, {id: "school", name: LANG.school, width: "10%", hidden: true, compute: function (A, B) {
  return g_spell_resistances[A.school];
}, sortFunc: function (B, A, C) {
  return strcmp(g_spell_resistances[B.school], g_spell_resistances[A.school]);
}}, {id: "reagents", name: LANG.reagents, align: "left", width: "15%", compute: function (F, C) {
...
}}, {id: "source", name: LANG.source, width: "12%", hidden: true, compute: function (B, D) {
  if (B.source != null) {
    for (var C = 0, A = B.source.length; C < A; ++C) {
      if (C > 0) {
        D.appendChild(document.createTextNode(LANG.comma));
      }
      D.appendChild(document.createTextNode(g_sources[B.source[C]]));
    }
  }
}, sortFunc: function (B, A, C) {
  return Listview.funcBox.assocArrCmp(B.source, A.source, g_sources);
}}, {id: "skill", name: LANG.skill, width: "16%", compute: function (E, C) {
  if (E.skill != null) {
    var B = ce("div");
    B.className = "small";
    for (var D = 0, G = E.skill.length; D < G; ++D) {
      if (D > 0) {
        B.appendChild(document.createTextNode(LANG.comma));
      }
      if (E.skill[D] == -1) {
        B.appendChild(document.createTextNode(LANG.ellipsis));
      } else {
        if (in_array([7, -2, -3, 11, 9], E.cat) != -1) {
          var H = ce("a");
          H.className = "q1";
          H.href = "?spells=" + E.cat + "." + (E.chrclass ? E.chrclass + "." : "") + E.skill[D];
          H.appendChild(document.createTextNode(g_spell_skills[E.skill[D]]));
          B.appendChild(H);
        } else {
          B.appendChild(document.createTextNode(g_spell_skills[E.skill[D]]));
        }
      }
    }
    if (E.learnedat > 0) {
      ...
      if (E.learnedat == 9999) {
        ...
        if (E.learnedat > 0) {
          F.appendChild(document.createTextNode(E.learnedat));
      ...
      if (I > 0) {
        ...
        for (var D = 0; D < A.length; ++D) {
          if (A[D] > 0) {
            ...
            var J = ce("span");
            J.className = "r" + (D + 1);
            J.appendChild(document.createTextNode(A[D]));
            B.appendChild(J);
          }
        }
        C.appendChild(B);
  ...
  return "?spell=" + A.id;
}}, zone: {sort: [1], nItemsPerPage: -1, columns: [{id: "name", name: LANG.name, align: "left", value: "name", compute: function (B, D) {
  var A = ce("a");
  A.style.fontFamily = "Verdana, sans-serif";
  A.href = this.template.getItemLink(B);
  A.appendChild(document.createTextNode(B.name));
...
}}, {id: "level", name: LANG.level, width: "10%", compute: function (A, B) {
  if (A.minlevel > 0 && A.maxlevel > 0) {
    if (A.minlevel != A.maxlevel) {
      return A.minlevel + LANG.hyphen + A.maxlevel;
    } else {
...
}}, {id: "territory", name: LANG.territory, width: "13%", compute: function (A, C) {
  var B = ce("span");
  switch (A.territory) {
    case 0:
      B.className = "alliance-icon";
  ...
  B.appendChild(document.createTextNode(g_zone_territories[A.territory]));
  C.appendChild(B);
}, sortFunc: function (B, A, C) {
  return strcmp(g_zone_territories[B.territory], g_zone_territories[A.territory]);
}}, {id: "instancetype", name: LANG.instancetype, compute: function (A, D) {
  if (A.instance > 0) {
    var B = ce("span");
...
}}, {id: "category", name: LANG.category, width: "15%", compute: function (B, C) {
  C.className = "small q1";
  var A = ce("a");
  A.href = "?zones=" + B.category;
  A.appendChild(document.createTextNode(g_zone_categories[B.category]));
  C.appendChild(A);
}, sortFunc: function (B, A, C) {
  return strcmp(g_zone_categories[B.category], g_zone_categories[A.category]);
}}], getItemLink: function (A) {
  return "?zone=" + A.id;
}}, comment: {sort: [1], mode: 2, nItemsPerPage: 40, poundable: 2, columns: [{value: "number"}, {value: "id"}, {value: "rating"}], compute: function (A, R) {
  var l, h = new Date(A.date), F = (g_serverTime - h) / 1e3, j = (g_user.roles & 26) != 0, I = A.rating < 0 || A.purged || A.deleted, X = j || A.user.toLowerCase() == g_user.name.toLowerCase(), K = X && A.deleted == 0, G = X && A.replyTo != A.id, g = A.purged == 0 && A.deleted == 0 && g_user.id && A.user.toLowerCase() != g_user.name.toLowerCase() && in_array(A.raters, g_user.id, function (n) {
    return n[0];
  }) == -1, m = A.rating >= 0 && (g_user.id == 0 || g);
  A.ratable = g;
  R.className = "comment";
  if (A.indent) {
  ...
  if (I) {
    ...
    J.appendChild(document.createTextNode(LANG.lvcomment_show));
    L.appendChild(J);
    L.appendChild(document.createTextNode(" " + " " + " "));
  }
  var C = ce("b");
  C.appendChild(document.createTextNode(LANG.lvcomment_rating));
  var U = ce("span");
  U.appendChild(document.createTextNode((A.rating > 0 ? "+" : "") + A.rating));
  C.appendChild(U);
  L.appendChild(C);
  L.appendChild(document.createTextNode(" "));
  var O = ce("span");
  ...
  V.appendChild(L);
  V.appendChild(document.createTextNode(LANG.lvcomment_by));
  var B = ce("a");
  B.href = "?user=" + A.user;
  B.appendChild(document.createTextNode(A.user));
  V.appendChild(B);
  ...
  V.appendChild(M);
  V.appendChild(document.createTextNode(LANG.lvcomment_patch1 + g_getPatchVersion(h) + LANG.lvcomment_patch2));
  R.appendChild(V);
  N.className = "comment-body" + Listview.funcBox.coGetColor(A);
  if (A.indent) {
    N.className += " comment-body-indent";
  }
  N.innerHTML = Markup.toHtml(A.body, {mode: Markup.MODE_COMMENT});
  R.appendChild(N);
  if ((A.roles & 26) == 0 || g_user.roles & 26) {
    var e = ce("div");
    A.divLastEdit = e;
    e.className = "comment-lastedit";
    e.appendChild(document.createTextNode(LANG.lvcomment_lastedit));
    var Z = ce("a");
  ...
  if (X) {
    var E = ce("span");
    var Y = ce("a");
    Y.appendChild(document.createTextNode(LANG.lvcomment_edit));
  ...
  if (K) {
    var k = ce("span");
    var P = ce("a");
    P.appendChild(document.createTextNode(LANG.lvcomment_delete));
  ...
  if (G) {
    var D = ce("span");
    var T = ce("a");
    T.appendChild(document.createTextNode(LANG.lvcomment_detach));
  ...
  var S = ce("a");
  S.appendChild(document.createTextNode(LANG.lvcomment_reply));
  if (g_user.id > 0) {
  ...
  A.appendChild(document.createTextNode(LANG.lvcomment_add));
  E.appendChild(A);
  var D = ce("span");
  ...
  D.appendChild(document.createTextNode("|"));
  E.appendChild(D);
  E.appendChild(document.createTextNode(LANG.lvcomment_sort));
  var C = ce("a");
  C.href = "javascript:;";
  C.appendChild(document.createTextNode(LANG.lvcomment_sortdate));
  C.onclick = Listview.funcBox.coSortDate.bind(this, C);
  E.appendChild(C);
  E.appendChild(document.createTextNode(LANG.comma));
  var B = ce("a");
  B.href = "javascript:;";
  B.appendChild(document.createTextNode(LANG.lvcomment_sortrating));
...
}}, commentpreview: {sort: [3], nItemsPerPage: 75, columns: [{id: "subject", name: LANG.subject, align: "left", value: "subject", compute: function (D, C) {
  var A = ce("a");
  A.style.fontFamily = "Verdana, sans-serif";
  A.href = this.template.getItemLink(D);
  A.appendChild(document.createTextNode(D.subject));
  C.appendChild(A);
  var B = ce("div");
  B.className = "small";
  B.appendChild(document.createTextNode(LANG.types[D.type][0]));
  C.appendChild(B);
}}, {id: "preview", name: LANG.preview, align: "left", width: "50%", value: "preview", compute: function (H, G) {
  var E = ce("div");
  E.className = "crop";
  if (H.rating >= 10) {
    E.className += " comment-green";
  }
  E.appendChild(document.createTextNode(Markup.removeTags(H.preview, {mode: H.rating != null ? Markup.MODE_COMMENT : Markup.MODE_ARTICLE})));
  G.appendChild(E);
  var A = H.rating != null;
  var D = H.user != null;
  if (A || D) {
    ...
    E.className = "small3";
    if (D) {
      E.appendChild(document.createTextNode(LANG.lvcomment_by));
      var B = ce("a");
      B.href = "?user=" + H.user;
      B.appendChild(document.createTextNode(H.user));
      E.appendChild(B);
      if (A) {
        E.appendChild(document.createTextNode(LANG.hyphen));
      }
    }
    if (A) {
      E.appendChild(document.createTextNode(LANG.lvcomment_rating + (H.rating > 0 ? "+" : "") + H.rating));
      var C = ce("span"), F = "";
      C.className = "q7";
      if (H.deleted) {
        F = " (Deleted)";
  ...
  var B = new Date(E.date), A = (g_serverTime - B) / 1e3;
  var C = ce("span");
  Listview.funcBox.coFormatDate(C, A, B, 0, 1);
  D.appendChild(C);
}}], getItemLink: function (A) {
  return "?" + g_types[A.type] + "=" + A.typeId + (A.id != null ? "#comments:id=" + A.id : "");
}}, screenshot: {sort: [], mode: 3, nItemsPerPage: 40, nItemsPerRow: 4, poundable: 2, columns: [], compute: function (J, C, E) {
  var N, G = new Date(J.date), O = (g_serverTime - G) / 1e3;
  C.className = "screenshot-cell";
  C.vAlign = "bottom";
  var M = ce("a");
  M.href = "#screenshots:id=" + J.id;
  M.onclick = rf2;
  var F = ce("img"), B = Math.min(150 / J.width, 150 / J.height);
  ...
  if (P) {
    M = ce("a");
    M.href = "?user=" + J.user;
    M.appendChild(document.createTextNode(J.user));
    I.appendChild(document.createTextNode(LANG.lvscreenshot_from));
    I.appendChild(M);
    I.appendChild(document.createTextNode(" "));
  }
  var Q = ce("span");
  ...
  if (H || D) {
    ...
    if (D) {
      var L = ce("small");
      L.appendChild(document.createTextNode(LANG.types[J.type][0] + LANG.colon));
      var K = ce("a");
      K.appendChild(document.createTextNode(J.subject));
    ...
    if (H) {
      ...
      A.appendChild(document.createTextNode('"' + J.caption + '"'));
    }
    I.appendChild(A);
  }
  ...
  if (typeof g_pageInfo == "object" && g_pageInfo.type > 0) {
    ...
    A.appendChild(document.createTextNode(LANG.lvscreenshot_submit));
    B.appendChild(A);
    C.appendChild(B);
  }
}, onNoData: function (C) {
...
var g_spells = [];
var g_users = [];
var g_types = {1: "npc", 2: "object", 3: "item", 4: "itemset", 5: "quest", 6: "spell", 7: "zone", 8: "faction"};
var g_locales = {0: "enus", 2: "frfr", 3: "dede", 6: "eses", 25: "wotlk"};
...
var $WowheadPower = new function () {
  var V, a, L, T, F, J, H, R = 0, i = 0, K = 1, E = 2, M = 3, N = 4, G = 3, O = 5, g = 6, j = 15, W = 15, S = {3: [g_items, "item"], 5: [g_quests, "quest"], 6: [g_spells, "spell"]};
  function Z() {
...
LiveSearch = new function () {
  var currentTextbox, lastSearch = {}, lastDiv, timer, prepared, container, cancelNext, hasData;
  ...
  function display(textbox, search, suggz, dataz) {
    ...
    if (!Browser.ie6) {
      container.appendChild(ce("em"));
      container.appendChild(ce("var"));
      container.appendChild(ce("strong"));
    }
    search = search.replace(/[^a-z0-9'\-]/, " ");
    search = search.replace(/\s+/g, " ").replace(/(^\s*|\s*$)/g, "");
    var parts = search.split(" "), strRegex = "";
    ...
    for (var i = 0, len = suggz.length; i < len; ++i) {
      ...
      sp.appendChild(document.createTextNode(LANG.types[type][0]));
      a.appendChild(sp);
      var buffer = suggz[i];
      ...
      a.appendChild(sp2);
      if (type == 6 && param2) {
        a.appendChild(document.createTextNode(" (" + param2 + ")"));
      }
      div2.appendChild(a);
  ...
  function receive(xhr, opt) {
    ...
    if (text == "0" || text.charAt(0) != "[" || text.charAt(text.length - 1) != "]") return;
    text = text.replace("\n", "");
    var a = eval(text);
    var search = a[0];
  ...
  function fetch(textbox, search) {
    search = search.trim();
    new Ajax("opensearch.php?search=" + urlencode(search), {onSuccess: receive, textbox: textbox, search: search});
  }
  ...
  function onKeyUp(e) {
    ...
    var search = textbox.value.replace(/\s+/g, " ").replace(/(^\s*|\s*$)/g, "");
...
Lightbox = new function () {
  ...
  function I() {
    ...
    P = 1;
    var R = document.getElementById("layers");
    F = ce("div");
  ...
  function M() {
    var S = g_getScroll().y, R = g_getWindowSize().h;
  ...
  function G() {
    ...
    E = 0;
    F.style.display = N.style.display = "none";
    array_apply(document.getElementsByTagName("iframe"), function (R) {
      R.style.display = "";
    });
  }
  function L() {
  ...
  this.show = function (V, S, R) {
    E = 1;
    C = S || {};
    array_apply(document.getElementsByTagName("iframe"), function (W) {
...
function toggle_filters() {
  ...
  if (document.getElementById("filters_form").classList.contains("filters_hidden")) filters.setAttribute("value", "off"); else filters.setAttribute("value", "on");
}
